...
const bookAppointment = async (req, res) => {
  ...
  try {
    ...
    const scheduleResult = await pool.query(queries.getScheduleById, [scheduleId]);
    ...
    res.status(200).json({ message: 'Appointment booked successfully', data: result.rows[0] });
  } catch (error) {
    await pool.query('ROLLBACK');
